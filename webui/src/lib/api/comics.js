...
export async function getComic(libraryId, comicId) {
	const comic = await api.get(`/libraries/${libraryId}/comics/${comicId}`);
...
export async function getComicPage(libraryId, comicId, page) {
	...
	return api.getBlob(`/libraries/${libraryId}/comics/${comicId}/pages/${pageIndex}/remote`, {
		cache: 'no-store'
	});
}
 ...
 * Uses the Kottlib-native JSON API.
 */
export async function updateReadingProgress(libraryId, comicId, currentPage) {
	// Convert 1-based page to 0-based index for API
	const pageIndex = Math.max(0, currentPage - 1);

	return api.post(`/libraries/${libraryId}/comics/${comicId}/progress`, {
...
export async function getPreviousComic(libraryId, comicId) {
	return api.get(`/libraries/${libraryId}/comics/${comicId}/previous`);
}
...
export async function getNextComic(libraryId, comicId) {
	return api.get(`/libraries/${libraryId}/comics/${comicId}/next`);
}
