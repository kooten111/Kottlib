<script>
	...
	import { Scan, Settings, PlayCircle, CheckCircle, XCircle, AlertCircle } from 'lucide-svelte';
	...
	async function loadScannerData() {
		try {
			...
			if (!testLibraryId && libraryConfigs.length > 0) {
				...
				const configuredLib = libraryConfigs.find(lib => lib.primary_scanner);
				testLibraryId = configuredLib ? configuredLib.library_id : libraryConfigs[0].library_id;
			}
	...
	function getConfidenceBadgeClass(level) {
		...
		return classes[level] || 'bg-gray-800 text-gray-200 border border-gray-600';
	}
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
	...
	{:else}
		...
		<Card class="mb-6">
			...
			<div class="grid gap-4 md:grid-cols-2">
				{#each availableScanners as scanner}
					<div class="border border-gray-700 rounded-lg p-4 bg-dark-bg-tertiary hover:border-gray-600 transition-colors">
						<div class="flex items-start justify-between">
							<div>
								<h3 class="font-medium text-lg text-dark-text">{scanner.name}</h3>
								<span class="text-sm text-dark-text-secondary">Level: {scanner.scan_level}</span>
							</div>
							<span class="px-2 py-1 bg-blue-900 text-blue-200 text-xs rounded-full border border-blue-700">
								Active
							</span>
						</div>
						<p class="mt-2 text-sm text-dark-text-secondary">{scanner.description || 'No description'}</p>
						{#if scanner.config_keys && scanner.config_keys.length > 0}
							<div class="mt-2">
								<p class="text-xs text-dark-text-muted">Config options:</p>
								<div class="flex flex-wrap gap-1 mt-1">
									{#each scanner.config_keys as key}
										<span class="text-xs bg-dark-bg-secondary border border-gray-700 px-2 py-0.5 rounded text-dark-text-secondary">{key}</span>
									{/each}
								</div>
							</div>
						{/if}
		...
		<Card class="mb-6">
			<h2 class="text-xl font-semibold mb-4 text-dark-text">Library Scanner Configuration</h2>
			{#if libraryConfigs.length === 0}
				<div class="text-center py-8 text-dark-text-secondary">
					<p>No libraries found.</p>
					<p class="text-sm mt-2">Create a library first to configure scanners.</p>
				</div>
			{:else}
				<div class="space-y-4">
					{#each libraryConfigs as config}
						<div class="border border-gray-700 rounded-lg p-4 bg-dark-bg-tertiary hover:border-gray-600 transition-colors">
							<div class="flex items-center justify-between mb-3">
								<div>
									<h3 class="font-medium text-dark-text">{config.library_name}</h3>
									<p class="text-xs text-dark-text-secondary mt-0.5">{config.library_path}</p>
								</div>
								<button
							...
							<div class="grid grid-cols-2 gap-4 text-sm">
								<div>
									<span class="text-dark-text-secondary">Primary Scanner:</span>
									<span class="ml-2 font-medium text-dark-text">
										{#if config.primary_scanner}
											{config.primary_scanner}
										{:else}
											<span class="text-status-warning">Not configured</span>
										{/if}
									</span>
								</div>
								<div>
									<span class="text-dark-text-secondary">Confidence Threshold:</span>
									<span class="ml-2 font-medium text-dark-text">{(config.confidence_threshold * 100).toFixed(0)}%</span>
								</div>
								{#if config.fallback_scanners && config.fallback_scanners.length > 0}
									<div class="col-span-2">
										<span class="text-dark-text-secondary">Fallback Scanners:</span>
										<span class="ml-2 font-medium text-dark-text">{config.fallback_scanners.join(', ')}</span>
									</div>
								{/if}
							</div>
						</div>
						...
						<div class="flex items-start gap-3">
							...
							<div class="flex-1">
								<div class="flex items-center justify-between mb-2">
									<p class="font-medium text-green-200">Match Found!</p>
									<span class="px-2 py-1 rounded-full text-xs font-medium {getConfidenceBadgeClass(testResult.confidence_level)}">
										{testResult.confidence_level} ({(testResult.confidence * 100).toFixed(0)}%)
									</span>
								</div>
								<div class="space-y-2 text-sm">
									<div>
										<span class="text-dark-text-secondary">Source:</span>
										<a href={testResult.source_url} target="_blank" class="ml-2 text-accent-blue hover:text-blue-400 hover:underline">
											{testResult.source_url}
										</a>
									</div>
									<div>
										<span class="text-dark-text-secondary">Title:</span>
										<span class="ml-2 text-dark-text">{testResult.metadata.title || 'N/A'}</span>
									</div>
									{#if testResult.metadata.artists && testResult.metadata.artists.length > 0}
										<div>
											<span class="text-dark-text-secondary">Artists:</span>
											<span class="ml-2 text-dark-text">{testResult.metadata.artists.join(', ')}</span>
										</div>
									{/if}
									{#if testResult.tags && testResult.tags.length > 0}
										<div>
											<span class="text-dark-text-secondary">Tags:</span>
											<div class="flex flex-wrap gap-1 mt-1">
												{#each testResult.tags.slice(0, 10) as tag}
													<span class="text-xs bg-dark-bg-secondary border border-gray-700 px-2 py-0.5 rounded text-dark-text-secondary">
														{tag.split(':')[1] || tag}
													</span>
												{/each}
												{#if testResult.tags.length > 10}
													<span class="text-xs text-dark-text-muted">
														+{testResult.tags.length - 10} more
													</span>
												{/if}
											</div>
										</div>
		...
		<Card>
			...
			<div class="space-y-4">
				...
				{#if bulkResults}
					<div class="border border-gray-700 rounded-lg p-4 bg-dark-bg-tertiary">
						<div class="grid grid-cols-3 gap-4 mb-4">
							<div class="text-center">
								<p class="text-2xl font-bold text-dark-text">{bulkResults.total}</p>
								<p class="text-sm text-dark-text-secondary">Total</p>
							</div>
							<div class="text-center">
								<p class="text-2xl font-bold text-status-success">{bulkResults.matched}</p>
								<p class="text-sm text-dark-text-secondary">Matched</p>
							</div>
							<div class="text-center">
								<p class="text-2xl font-bold text-status-error">{bulkResults.rejected}</p>
						...
						<div class="space-y-2 max-h-96 overflow-y-auto">
							{#each bulkResults.results as result}
								<div class="flex items-center gap-2 text-sm bg-dark-bg-secondary border border-gray-700 p-2 rounded">
									{#if result.status === 'matched'}
										<CheckCircle class="w-4 h-4 text-status-success flex-shrink-0" />
										<span class="flex-1 truncate text-dark-text">{result.query}</span>
										<span class="px-2 py-0.5 rounded text-xs {getConfidenceBadgeClass(result.confidence_level)}">
											{(result.confidence * 100).toFixed(0)}%
										</span>
									{:else}
										<XCircle class="w-4 h-4 text-status-error flex-shrink-0" />
										<span class="flex-1 truncate text-dark-text-secondary">{result.query}</span>
										<span class="text-xs text-dark-text-muted">{result.reason || result.error || 'Failed'}</span>
									{/if}
								</div>
							{/each}
						</div>
...
{#if showConfigModal && configLibrary}
	<div class="fixed inset-0 flex items-center justify-center z-50 p-4" style="background: var(--color-overlay); backdrop-filter: blur(4px);">
		<div class="bg-dark-bg-secondary rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border-2 border-gray-700">
			<!-- Modal Header -->
			<div class="border-b border-gray-700 px-6 py-4 flex items-center justify-between sticky top-0 bg-dark-bg-secondary">
				<h3 class="text-xl font-semibold text-dark-text">Configure Scanner for {configLibrary.library_name}</h3>
				<button
			...
			<div class="px-6 py-4 space-y-6">
				...
				<div>
					...
					>
						...
						{#each availableScanners as scanner}
							<option value={scanner.name}>{scanner.name} - {scanner.description}</option>
						{/each}
					</select>
					<p class="text-xs text-dark-text-secondary mt-1">The primary scanner to use for this library</p>
				</div>
				...
				<div>
					...
					/>
					<div class="flex justify-between text-xs text-dark-text-secondary mt-1">
						<span>0% (Accept all)</span>
						<span>50% (Balanced)</span>
						<span>100% (Perfect only)</span>
					</div>
					<p class="text-xs text-dark-text-secondary mt-2">
						Minimum confidence score required to accept a match. Results below this threshold will be rejected.
					</p>
				</div>
				...
				<div>
					...
					<p class="text-xs text-dark-text-secondary mt-2">
						If primary scanner confidence is below this threshold, fallback scanners will be used (if configured).
					</p>
				</div>
						...
						{#each availableScanners as scanner}
							{#if scanner.name !== configForm.primary_scanner}
								<label class="flex items-center">
									<input
										type="checkbox"
										checked={configForm.fallback_scanners.includes(scanner.name)}
										on:change={(e) => {
											if (e.target.checked) {
												configForm.fallback_scanners = [...configForm.fallback_scanners, scanner.name];
											} else {
												configForm.fallback_scanners = configForm.fallback_scanners.filter(s => s !== scanner.name);
											}
										}}
									...
									/>
									<span class="ml-2 text-sm text-dark-text">{scanner.name} - {scanner.description}</span>
								</label>
							{/if}
						{/each}
					</div>
					<p class="text-xs text-dark-text-secondary mt-2">
						Additional scanners to try if the primary scanner returns low confidence results.
					</p>
				</div>
				...
				{#if configError}
					<div class="bg-red-950 border border-red-800 rounded-lg p-4 flex items-start gap-3">
						<XCircle class="w-5 h-5 text-status-error flex-shrink-0 mt-0.5" />
						<div>
							<p class="font-medium text-red-200">Configuration Error</p>
							<p class="text-sm text-red-300 mt-1">{configError}</p>
						</div>
					</div>
				{/if}
			</div>
			...
			<!-- Modal Footer -->
			<div class="border-t border-gray-700 px-6 py-4 flex justify-end gap-3 sticky bottom-0 bg-dark-bg-secondary">
				<button
					on:click={closeConfigModal}
